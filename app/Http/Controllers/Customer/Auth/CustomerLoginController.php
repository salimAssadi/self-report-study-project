...
class CustomerLoginController extends Controller
{
    ...
    public function showLoginForm($slug)
    {
        try {
            ...
            return view('storefront.' . $store->theme_dir .'.user.login', compact('store', 'slug', 'settings', 'total_item', 'categories', 'view'));
        } catch (\Throwable $th) {
            return redirect()->route('dashboard')->with('error', __('Store not found.'));
        }
    }
    ...
    public function profile($slug, $id)
    {
        ...
        return view('storefront.'.$store->theme_dir.'.customer.profile', compact('slug', 'store', 'userDetail'));
    }
